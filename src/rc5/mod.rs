...
impl RC5 {
    pub fn new(config: RC5Config, key : Vec<u8>) -> Result<Self, RC5Error> {
        let table_size : usize = (2 * (config.number_of_rounds + 1)).into();
        let mut rc5 = Self {
            config,
            table_size,
...
mod tests {
    ...
    fn setup_tests(key : Vec<u8>, number_of_rounds : u8, bytes_in_key : u8) -> RC5 {
        let config = match RC5Config::new(32, number_of_rounds, bytes_in_key) {
            Ok(res) => res,
            Err(e) => panic!("Error: {:?}", e),
        };
        let rc5 = match RC5::new(config, key) {
            Ok(res) => res,
            Err(e) => panic!("Error: {:?}", e),
        };
